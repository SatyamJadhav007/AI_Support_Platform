  ...
  handler: async (ctx): Promise<Vapi.PhoneNumbersListResponseItem[]> => {
    ...
    if (!secretData.privateApiKey) {
      throw new ConvexError({
        code: "NOT_FOUND",
        message:
          "Credentials incomplete. No private API key found. Please reconnect your Vapi account.",
      });
    }
    if (!secretData.publicApiKey) {
      throw new ConvexError({
        code: "NOT_FOUND",
        message:
          "Credentials incomplete. No public API key found. Please reconnect your Vapi account.",
      });
    }
    const vapiClient = new VapiClient({
      token: secretData.privateApiKey,
