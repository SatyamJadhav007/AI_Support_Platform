...
import { InfinitScrollTrigger } from "@workspace/ui/components/infinite-scroll-trigger";
import { cn } from "@workspace/ui/lib/utils";
import { Skeleton } from "@workspace/ui/components/skeleton";
const formSchema = z.object({
  message: z.string().min(1, "Message is requiered"),
});
...
export const ConversationIdView = ({
  ...
  const handleToggleStatus = async () => {
    ...
    // what is the statue cycle??
    // unresovled->escalated->resolved->unresolved
  ...
  return (
    <div className="flex h-full flex-col bg-muted">
      ...
      <AIConversation className="max-h-[calc(100vh-180vh)]">
        <AIConversationContent>
          ...
          {toUIMessages(messages.results ?? [])?.map((message) => (
            <AIMessage
              //It is reverse because we are watching from the assistant perspective
